...
- Final local regression: Server and Client `go test -race ./...`, `make lint`, `make build`, `make contract`, `make security`, 2-second fuzz targets, PERF-001/002, fixed FRPC/FRPS v0.68.0 `verify`, checksum validation, native FRPS/FRPC TCP network E2E and the opt-in real FRPS/FRPC + Plugin metadata E2E all pass. The release build emits only `frp-panel-server` and `frp-panel-client`; the backup restore command remains a Server source-level operational tool. Vite reports a non-blocking warning for the current JavaScript chunk size. Docker image build/scan was not locally executable because the current Docker daemon was unavailable, so it remains an external CI/release gate.
...
Linux/production FRPS/FRPC Plugin compatibility matrix, Cloudflare Sandbox timeout compensation, ACME Staging against a real CA, Router TLS SNI certificate hot reload, a clean-host disaster restore drill, PERF-003~007 target-scale measurements, implementation-level request/response Contract tests beyond route parity, formal SAST/SCA/CI attestations, cosign signing, and a production HTTPS deployment remain release gates. Local tests never mark those external integrations active.
