...
- `scripts/validate-openapi.rb` validates OpenAPI 3.1 paths, unique operation IDs and the WebSocket metadata; `make sbom` emits a local SPDX inventory and `make checksums` emits SHA-256 checksums for the two release artifacts. Artifact signing and third-party scanner attestations remain release work.
- Final local regression: Server and Client `go test -race ./...`, repository `make test lint build contract sbom checksums`, whitespace validation and secret-pattern scan pass. The release build emits only `frp-panel-server` and `frp-panel-client`; the backup restore command remains a Server source-level operational tool. Vite reports a non-blocking warning for the current JavaScript chunk size.
