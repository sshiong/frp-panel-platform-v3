...
export SERVER_TLS_CERT_FILE=
export SERVER_TLS_KEY_FILE=
...
export FRP_ROUTER_TLS_ENABLED=true
...
In production the Server control process itself requires `SERVER_TLS_CERT_FILE` and `SERVER_TLS_KEY_FILE` and serves HTTPS; a terminating reverse proxy may be placed in front as an additional boundary. A non-loopback `FRP_ROUTER_LISTEN_ADDR` also requires `FRP_ROUTER_TLS_ENABLED=true`; a loopback Router behind a separately managed TLS reverse proxy may remain internal HTTP. Configure an IP SAN, trusted CA, or explicit certificate fingerprint trust for Client Panel users connecting by IP. The initial admin credential is delivered through the protected `initial-admin.txt` file; it is never meant to be a long-lived deployment secret.
