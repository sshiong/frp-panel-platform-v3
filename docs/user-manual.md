...
验证 `Token.Verify`、`Zone.Read` 和 `DNS.Read` 能力，并显示可访问 Zone 清单；`DNS.Write`
不会在用户 Zone 上做破坏性探测，会明确显示为“待安全写入验证”，直到实际 DNS
写入成功或被 Cloudflare 拒绝。
...
清除 Token 同样需要在线、3 秒倒计时、确认和 reauth；服务端会退休并擦除密文、Nonce
和 Zone 权限缓存，取消尚未执行的 Token 验证任务；已有 DNS 记录不会自动删除。
