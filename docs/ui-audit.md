...
| Theming | 3/4 | Both panels now have explicit graphite/ivory/steel-blue/state-color aliases in independent `tokens.css` files, with repeated surfaces, inputs, navigation, dialogs and borders consuming semantic variables. Fonts use a no-network fallback stack and production CSS rejects external assets. Intentional one-off state shades remain local. |
| Responsive design | 4/4 | Admin and Client were checked at 390×844 with Playwright; `document.documentElement.scrollWidth` equals `window.innerWidth` (390px), mobile layouts stack, login grids use zero-minimum columns under system-font fallback, tables scroll within their panels, and key controls receive 44px touch targets. |
...
### Resolved — External font dependency in embedded builds

- **Location:** `web/admin/src/style.css`, `web/client/src/style.css`, and `scripts/css-token-policy.rb`.
- **Category:** Release boundary / security headers.
- **Finding:** The panels previously imported Google Fonts at runtime, while the production CSP permits same-origin resources only. This made the embedded release dependent on an external request that CSP would reject.
- **Fix:** Removed the external imports and added a CSS policy that rejects external `url()` assets in production sources. The existing named font stack now falls back locally without network access.
- **Evidence after fix:** `make contract`, `make lint`, `make test`, `make build`, and `make accessibility` pass; built Admin/Client assets contain no external font imports.
