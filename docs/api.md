...
- Cloudflare DNS operations use the provider interface and the HTTP adapter supports token verification, paginated zone discovery, create/update and delete. Token replacement is a two-phase `pending → verified_pending → valid` flow: verification stores the separate Token/Zone/DNS capability results, accessible Zone list, and domains that would lose Zone access; `DNS.Write` remains explicitly “unchecked” until a safe real DNS write succeeds or is denied. Only an explicit reauthenticated activation switches `users.active_cloudflare_token_version`. The Client local API proxies status/upload/activation/clear to the Server and never caches the Token.
