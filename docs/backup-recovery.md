...
The backup endpoint requires a session-bound reauthentication ticket. The password is never stored in the database. The `backup.Decode` and `backup.Restore` library functions verify the package header, decrypt the archive, check every manifest checksum and SQLite integrity, and install the snapshot atomically. If a target database already exists it is renamed to a timestamped `.before-restore-*` path for recovery. If post-install session invalidation or protected-file restoration fails, the new database is removed and the previous target is restored before the error is returned. Restore revokes all restored sessions and runtime credentials; the next Server startup schedules a Router rebuild. Stop the Server Panel before restore and produce a recovery report before returning the instance to service. JSON export is for non-sensitive mapping/domain previews only and is not a full restore format.
