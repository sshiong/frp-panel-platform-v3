...
| 2026-08-10 | 外部收集器逐步骤证据结构化 | 已实现并通过回归；本地、阻塞和跳过步骤统一保存标准要求的 `environment`、`steps`、`expected`、`actual`、`artifacts`、`operator`、`executed_at` 字段，命令输出写入 0600 脱敏日志；`scripts/test-external-acceptance.rb` 增加字段与文件权限回归，缺少真实 Provider/目标环境/签名/负责人证据仍保持 blocked。 |
