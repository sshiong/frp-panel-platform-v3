...
| 2026-08-01 | Final hardening regression | 通过；Server/Client `go test -race ./...`、`make test lint build contract sbom checksums`、`git diff --check` 和敏感凭据模式扫描均通过；前端仅保留 Vite 大 chunk 警告 |
