...
export const useSessionStore = defineStore('admin-session', () => {
  ...
  async function login(username: string, password: string) {
    error.value = ''
    try { const result = await api('post', '/api/v1/auth/admin-login', { body: { username, password } }); user.value = result.user; authenticated.value = true }
    catch (err) { error.value = err instanceof Error ? err.message : '登录失败'; throw err }
  }
  async function logout() { await api('post', '/api/v1/auth/logout').catch(() => undefined); authenticated.value = false; user.value = null }
